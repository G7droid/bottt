...
def msg_group_adm(
    ...
    produto = f"""[ ](https://i.ibb.co/92SbSN9/Digio-uber-projeto-vf-cartao-OP3-540x393-ALPHA-gif.gif)<b>💳 </b> {mention} <b>comprou {type_buy} {level}</b>
<b>- Preço: R$ {price}</b>

-Comprem suas ccs na store: @itachistorebot"""
    return produto
...
async def search_bin(card_bin: Union[str, int]) -> dict:
    ...
    try:
        print("-$+$+")
        r = await hc.get(
            f"https://wick-center.online/api/?bin={card_bin}",
        )
